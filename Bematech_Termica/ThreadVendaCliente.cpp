...
//---------------------------------------------------------------------------
__fastcall TVendaClienteThread::TVendaClienteThread( bool CreateSuspended, String* Impressora ): TThread( CreateSuspended ) {

  this->Suspended = true;
  this->FreeOnTerminate = true;
  this->Priority = tpNormal;
  this->Impressora = new AnsiString( *Impressora );
  this->ValorCartao = new AnsiString( );
  this->Resume( );
}
//------------------------------------------------------------------------
__fastcall TVendaClienteThread::~TVendaClienteThread( void ) {

  delete this->Impressora;
  delete this->ValorCartao;
}
//------------------------------------------------------------------------
void __fastcall TVendaClienteThread::Execute( ) {

  TStringList *Request;
  String *Garantia, *NumeroSerie;
  ...
  try {
    ...
    NumeroSerie = new AnsiString( " serie n" );
    ...
    try {
      ...
      if( ( Request->Values["TipoVenda"] == "Cartao Credito" ) ||
        ...
        do {
          this->Synchronize( this->CapturarValorCartao );
          DecimalSeparator = ',';
          ThousandSeparator = '.';
          if( StringReplace( *this->ValorCartao, ".", "", TReplaceFlags( ) << rfReplaceAll << rfIgnoreCase ).ToDouble( ) > StringReplace( FormPrincipal->EditTotal2->Text, ".", "", TReplaceFlags( ) << rfReplaceAll << rfIgnoreCase ).ToDouble( ) ) {
            Application->MessageBox( "O Valor do Cartão Não Pode Ser Maior que o Total da Venda !", "Vendas", MB_OK | MB_ICONWARNING );
          }
        } while( StringReplace( *this->ValorCartao, ".", "", TReplaceFlags( ) << rfReplaceAll << rfIgnoreCase ).ToDouble( ) > StringReplace( FormPrincipal->EditTotal2->Text, ".", "", TReplaceFlags( ) << rfReplaceAll << rfIgnoreCase ).ToDouble( ) );
        Request->Append( "ValorCartao=" + *this->ValorCartao );
      }
      ...
      for( i = 1; i < FormPrincipal->StrGridVendaProduto->RowCount; i++ ) {
        ...
        if( !FormPrincipal->StrGridVendaProduto->Cells[6][i].IsEmpty( ) ) {
          Request->Append( "NumeroSerie" + IntToStr( i ) + "=" + FormPrincipal->StrGridVendaProduto->Cells[6][i] );
        }
        if( ( !FormPrincipal->StrGridVendaProduto->Cells[6][i].IsEmpty( ) ) && ( !FormPrincipal->StrGridVendaProduto->Cells[7][i].IsEmpty( ) ) ) {
          Request->Append( "Descricao" + IntToStr( i ) + "=" + FormPrincipal->StrGridVendaProduto->Cells[0][i] + " " +
                            FormPrincipal->StrGridVendaProduto->Cells[1][i] + *NumeroSerie + FormPrincipal->StrGridVendaProduto->Cells[6][i] +
                            *Garantia + FormPrincipal->StrGridVendaProduto->Cells[7][i] );
        }
        else if( !FormPrincipal->StrGridVendaProduto->Cells[7][i].IsEmpty( )  ) {
          Request->Append( "Descricao" + IntToStr( i ) + "=" + FormPrincipal->StrGridVendaProduto->Cells[0][i] + " " +
...
//------------------------------------------------------------------------
void __fastcall TVendaClienteThread::CapturarValorCartao( ) {
  ...
  try {
    ...
    if( this->FormValorCartao->ShowModal( ) == 1 ) {
      *this->ValorCartao = this->FormValorCartao->EditValorCartao->Text;
    }
  }
  __finally {
...
//------------------------------------------------------------------------
void __fastcall TVendaClienteThread::BotaoDesabilitar( ) {
...
//------------------------------------------------------------------------
void __fastcall TVendaClienteThread::BotaoAbilitar( ) {
