...
__fastcall TVendaServidorThread::TVendaServidorThread( bool CreateSuspended, TStringList *VendaInfo ): TThread( CreateSuspended ) {
  ...
  this->CpfCnpjCliente = new AnsiString( );
...
void TVendaServidorThread::ImprimirVendaFiscal( ) {
  ...
  String *TipoVenda, *ValorTotal, *CpfCnpj, *NomeCliente;
  String *Qtd, *PrecoUnit, *Codigo, *Descricao, *TaxaIcms;
  bool *LeituraX;
  ...
  try {
    ...
    *FormPrincipal->ImpressoraEcfSemaforo = false;
    ...
    TaxaIcms = new AnsiString( "18,00" );
    //CpfCnpj = new AnsiString( );
    //NomeCliente = new AnsiString( );

    ImpressoraEcf = new TImpressoraFiscalBematech( );

    try {
      /*
      if( !ImpressoraEcf->AbrirCoxexaoSerial( this->Erro ) ) {
        throw( *this->Erro );
      }
      */
      if( !ImpressoraEcf->AbrirCupomFiscal( this->CpfCnpjCliente, this->Erro ) ) {
        throw( *this->Erro );
     }
      for( i = 1; i <= this->VendaInfo->Values["ItemQtd"].ToInt( ); i++ ) {
       ...
       if( !ImpressoraEcf->LancarItemFiscal( Qtd, PrecoUnit, TaxaIcms, Codigo, Descricao, this->Erro ) ) {
         throw( *this->Erro );
       }
      }
      if( !ImpressoraEcf->TotalizarCupom( this->Erro ) ) {
        throw( *this->Erro );
      }
      if( ( this->VendaInfo->Values["TipoVenda"] == "Dinheiro" ) ||
        ...
        *TipoVenda = "Financiado";
      }
      else if( ( this->VendaInfo->Values["TipoVenda"] == "Cartao Credito" ) ||
               ( this->VendaInfo->Values["TipoVenda"].SubString( 1, 15 ) == "Cartao Credito " ) ) {
        *TipoVenda = "Cartao Credito";
      }
      else if( ( this->VendaInfo->Values["TipoVenda"] == "Cartao Debito" ) ||
               ( this->VendaInfo->Values["TipoVenda"] == "Cheque Eletronico" ) ||
               ( this->VendaInfo->Values["TipoVenda"].SubString( 1, 18 ) == "Cheque Eletronico " ) ||
               ( this->VendaInfo->Values["TipoVenda"].SubString( 1, 14 ) == "Cartao Debito " ) ) {
        *TipoVenda = "Cartao Debito";
      }
      *ValorTotal = this->VendaInfo->Values["ValorTotal"];
      if( !ImpressoraEcf->Pagamento( TipoVenda, ValorTotal, Erro ) ) {
        throw( *this->Erro );
      }
      /*
      *ValorTotal = "0";
      if( !ImpressoraEcf->Pagamento( TipoVenda, ValorTotal, this->Erro ) ) {
        throw( *this->Erro );
      }
      */
      if( !ImpressoraEcf->FecharCupomFiscal( &( this->VendaInfo->Values["Vendedor"] ), this->Erro ) ) {
        throw( *this->Erro );
      }
...
//TImpressoraFiscalRfd *ImpressoraEcf;
  TImpressoraFiscalBematech *ImpressoraEcf;
  String *Relatorio, *Expandido, *TipoPagamento;
  ...
  try {
    ...
    try {
      TipoPagamento = new AnsiString( "Cartao Credito" );
      *this->Erro = "";
//    if( !ImpressoraEcf->AbrirCoxexaoSerial( this->Erro ) ) {
//      throw( *this->Erro );
//    }
//    if( *LeituraX ) {
//      if( !ImpressoraEcf->LeituraX( this->Erro ) ) {
//        throw( *this->Erro );
//      }
//    }
//    else {
      if( this->VendaInfo->Values["TipoVenda"] == "Cheque" ) {
      ...
      if( !ImpressoraEcf->AbrirCupomNaoFiscalVinculado( TipoPagamento, &(this->VendaInfo->Values["ValorTotal"]), this->NumeroCupom, this->Erro ) ) {
        throw( *this->Erro );
      }
//    }
      ...
      for( j = 0; j < 2; j++ ) {
        for( i = 0; i < this->CupomTef->Count; i++ ) {
          if( !ImpressoraEcf->ImprimirLinhaNaoFiscal( &( this->CupomTef->Strings[i] ), this->Erro ) ) {
            throw( *this->Erro );
          }
        }
      }
      /*
      if( !ImpressoraEcf->ImprimirTextoNaoFiscal( &( this->CupomTef->Text ), this->Erro ) ) {
        throw( *this->Erro );
      }
      */
      if( !ImpressoraEcf->FecharCupomNaoFiscal( this->Erro ) ) {
        throw( *this->Erro );
      }
    }
    catch( String &Mensagem ) {
...
//  Impressora->CarregarDll( this->Erro );
  ...
  for( j = 0; j < 2; j++ ) {
     Impressora->Iniciar( this->Erro );
      ...
      Impressora->AbrirCupom( this->NumeroCupom, this->CpfCnpjCliente, this->Erro );
      if( !this->Erro->IsEmpty( ) ) {
        throw( *this->Erro );
      }
      for( i = 1; i <= this->VendaInfo->Values["ItemQtd"].ToInt( ); i++ ) {
  ...
  try {
    ...
    *FormPrincipal->QuerySemaforo = false;
    ...
    try {
      ...
      */
      if( !ImpressoraEcf->ObterNumeroUltimoCupomFiscal( NumeroCupom, this->Erro ) ) {
        throw( *this->Erro );
      }
     ...
     if( this->VendaInfo->Values["Cliente"].SubString( 1, this->VendaInfo->Values["Cliente"].Length( ) - 2 ) != "Sem RG" ) {
        Query->SQL->Clear( );
        Query->SQL->Add( "SELECT Nome, Telefone, Cpf" );
        ...
        if( !Query->IsEmpty( ) ) {
          ...
          *this->Telefone = Query->FieldByName( "Telefone" )->AsString;
          if( !Query->FieldByName( "Cpf" )->AsString.IsEmpty( ) ) {
            *this->CpfCnpjCliente = Query->FieldByName( "Cpf" )->AsString;
          }
          else {
            *this->CpfCnpjCliente = this->VendaInfo->Values["Cliente"].SubString( 1, this->VendaInfo->Values["Cliente"].Length( ) - 2 );
          }
        }
        Query->Close( );
      }
